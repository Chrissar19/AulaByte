...
func procesar_correcto(destino: Control) -> void:
    ...
    var t := create_tween()
    t.tween_property(self, "global_position", cent_destino, 0.18).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
    t.parallel().tween_property(self, "scale", Vector2(0.0, 0.2), 0.25)
    await t.finished
    hide()
