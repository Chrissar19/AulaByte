# ====================================================================
# GAME MANAGER - AULABYTE
# ====================================================================
## Autoload central que gestiona el ciclo de vida, estados y progreso del juego
extends Node

# ====================================================================
# SEÑALES GLOBALES
# ====================================================================
## Notifica cambios en la salud para actualizar la interfaz.
signal vida_actualizada(nueva_vida: int)
signal jugador_muerto
signal nivel_completado
## Informa qué personaje fue elegido para instanciar el modelo correcto.
...
## Se dispara cuando se resuelve con éxito un reto
signal actividad_superada 

# ====================================================================
# CONSTANTES Y CONFIGURACIÓN
# ====================================================================
const RUTA_GUARDADO := "user://progreso_aulabyte.cfg"
const RUTA_MENU_PAUSA := "res://escenas/ui/menu_pausa.tscn"

## Define los momentos clave del flujo del juego.
enum EstadoJuego {
	MENU_PRINCIPAL,
	SELECCION_PERSONAJE,
	SELECCION_NIVEL,
...
const NOMBRES_ESTADOS := [
	"MENU_PRINCIPAL", "SELECCION_PERSONAJE", "SELECCION_NIVEL",
	"CARGANDO", "JUGANDO", "PAUSA", "MINIJUEGO",
	"TRANSICION_NIVEL", "GAME_OVER", "CREDITOS"
]
...
# ====================================================================
# VARIABLES DEL JUGADOR Y PROGRESO
# ====================================================================
...
# ====================================================================
# VARIABLES DE ACTIVIDAD (PIAGET / TOLT)
# ====================================================================
...
# MÁQUINA DE ESTADOS
# ====================================================================
## Administra la transición entre estados y activa la lógica de entrada/salida.
...
# ====================================================================
# GESTIÓN DE JUGADOR Y ESTADÍSTICAS
# ====================================================================
...
func reintentar_nivel_actual() -> void:
	# Solo reinicia vidas y restaura puntos al checkpoint del nivel
	reiniciar_vidas()
...
func obtener_personaje() -> personajeInfo:
...
func _on_minijuego_resuelto(exito: bool) -> void:
	...
	if exito:
		...
		if puerta_actual and is_instance_valid(puerta_actual):
			...
			if animacion_puerta > 0.0:
				await get_tree().create_timer(animacion_puerta).timeout
...
func establecer_parametros_actividad(nuevos_parametros: Dictionary) -> void:
	if nuevos_parametros.is_empty():
		return
	# Si los nuevos parámetros traen un "codigo", actualiza la variable interna
	if nuevos_parametros.has("codigo"):
		...
		if nuevo_cod is Array:
			...
			codigo_generado = true # Marca como generado para que no cree uno aleatorio
	# Actualiza las claves una por una
...
func generar_codigo(longitud: int = 4) -> Array[int]:
	# Solo genera si no hay un código ya establecido por el nivel
	if not codigo_generado or codigo_actividad_actual.is_empty():
		...
		# Guardamos en el diccionario de parámetros para la actividad
...
# DATOS Y ENTRADA
# =====================================================================
func obtener_tiempo() -> float:
	return tiempo_nivel_actual

func iniciar_tiempo(segundos: float) -> void:
	tiempo_restante = max(segundos, 0.0)
	tiempo_activo = true
	ultimo_segundo = int(tiempo_restante)
	emit_signal("tiempo_actualizado", ultimo_segundo)
	cambiar_estado(EstadoJuego.JUGANDO)
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("ui_cancel"):
		if estado_actual == EstadoJuego.JUGANDO:
			...
			# Cerrar menú de pausa (si existe) y reanudar
